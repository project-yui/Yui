const { existsSync, readdir, renameSync, rmSync, readFileSync, readdirSync } = require ("fs");
...
const windows = async () => {
    ...
    // 解压
    {
        ...
        if (!existsSync(path.resolve(programPath, 'wrapper.node'))) {
            log.info('extracting.....');
            if (!existsSync(path.resolve(programPath, 'Files'))) {
                execSync(`${_7zpath} x ${qqFilepath} -O${programPath} Files/versions/config.json -r`);
                const cfg = JSON.parse(readFileSync(path.resolve(programPath, 'Files/versions/config.json')).toString())
                execSync(`${_7zpath} x ${qqFilepath} -O${programPath} Files/versions/${cfg.curVersion}/resources/app -r`);
                moveFilesAndDeleteDir(path.resolve(programPath, `Files/versions/${cfg.curVersion}/resources/app`), programPath);
            }
        }
        else
        {
