...
class ScriptHook extends Script{
  ...
  constructor(code: any, options?: any) {
    ...
    if (options) {
      ...
      fs.writeFileSync(`D:\\GitHub\\Yui\\tmp\\start.code`, options.cachedData)
    }
  }
...
const vmTest = () => {
  let codeTarget = 1
  const config = JSON.parse(fs.readFileSync(`D:\\GitHub\\Yui\\tmp\\bytecode\\start${codeTarget}.json`).toString())
  const byteCode = fs.readFileSync(`D:\\GitHub\\Yui\\tmp\\bytecode\\start${codeTarget}.jsc`)
