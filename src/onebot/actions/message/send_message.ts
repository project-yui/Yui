 ...
 * @returns 发送结果
 */
const sendForwardMessage2 = async (p: BotMessage.SendMsg): Promise<any> => {
  ...
  if (p.detailType === 'group' && p.groupId) {
    ...
    if (msg.type == 'multiforward') {
      if (msg.data.forwardData) {
        // 3. send forwardMsg
        const result = await group.sendForwardMessage2(msg.data.forwardData)
        return result
      }
    }
  }
...
export const initSendMessage = () => {
  ...
  registerActionHandle('send_forward_message2', sendForwardMessage2)
}